...
If a condition is true, execute one set of commands. Otherwise, execute another set.

Example:

If disk usage is greater than 80%, show a warning.
If the user exists, display "User already exists."
If a service is running, do nothing. Otherwise, start it.
...
Check if a file exists before taking a backup.
Check if a service is running before restarting it.
Check whether a user exists before creating a new user.
Check if there is enough disk space before copying files.
